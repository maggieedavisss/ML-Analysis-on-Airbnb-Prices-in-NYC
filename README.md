...
Finally, we wanted to create a confidence interval for the best model –– which turned out to be the Random Forest Regression. Depending on the model we could have used bootstrapping or bagging, but the Random Forest regression makes it uniquely difficult to get a confidence interval. Fortunately, there is a Python package called `forestci` that is used for this exact purpose. You can input the regression model, the X_test values, and the X_train values to get a 95% confidence interval for each observation. As a proof of concept, we inputted the data from one of our homes to see the predicted market value and the confidence within which that falls. We found that the apartment we checked had a price of $387 because it is in a nice neighborhood and we listed the whole apartment. However, the range was between $288 at the 2.5% and $520 at the 97.5%. This is likely a result of the high price which skews log values and contains a sparse amount of data.
